...
class InsuranceCenterSimulation:
    ...
    def car_entry_event(self):
        self.data['Cars'][self.car[0]] = [self.clock, 0, 0, 1, 0, self.paired]  # Arrival time, Car Type, Has Complaint, Is Paired, Is complete case?, paired after calling pairing function?
        if random.random() < 0:
        ...
        if self.data['Cars'][self.car[0]][2] == 0:
            ...
            if self.state['Finishing Case Queue'] > 0:
                ...
                if self.state['Opening Case Queue'] > 0:
                    ...
                    new_event = {'Event Type': 'Assessment Entrance Event', 'Event Time': event_time, 'Car': [first_car_in_queue, 1]}
                    self.future_event_list.append(new_event)
