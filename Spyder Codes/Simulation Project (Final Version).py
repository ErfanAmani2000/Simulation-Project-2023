...
class InsuranceCenterSimulation:
    ...
    def __init__(self, simulation_time):
        ...
        self.paired = 1
        self.trace_list = []
        self.param = {'arrival_time': 3, 'Waiting in Parking': 30, 'Capacity': 40, 'N1': 2, 'N2': 3, 'N3': 2, 'N4': 1,
                      'LB': 5, 'M': 6, 'UB': 7, 'S2': 6, 'S3': 9, 'S4.l': 6, 'S4.m': 8, 'S4.h': 9, 'S6': 15}
    ...
    def starting_state(self):
        ...
        self.future_event_list.append({'Event Type': 'Car Entry Event', 'Event Time': 0, 'Car': [1, 1]})
    ...
    def data_server_calculater(self, name: str):
        """
         ...
         """
        self.data['Cumulative Stats']['Area Under Server Busy time'][name] += \
            self.state['{} Server Status'.format(name)] * (self.clock - self.data['Last Time Server Status Changed'][name])
    ...
    def car_entry_event(self):
        ...
        if (random.random() < 0) and (self.data['Cars'][self.car[0]][5] == 0):
    ...
    def filing_entrance_event(self):
        if self.data['Cars'][self.car[0]][4] == 1:
            ...
            if self.state['Filer Server Status'] == self.param["N2"]:
                ...
                event_time = self.clock + self.triangular(self.param["S4.l"], self.param["S4.m"], self.param["S4.h"])
            ...
            if self.state['Finishing Case Queue'] > 0:
                if self.state['Filer Server Status'] == self.param["N2"]:
                    ...
                    event_time = self.clock + self.triangular(self.param["S4.l"], self.param["S4.m"],
                                                              self.param["S4.h"])
                    new_event = {'Event Type': 'Exit From System', 'Event Time': event_time, 'Car': [self.car[0], 1]}
                    self.future_event_list.append(new_event)
            else:
    ...
    def exit_system(self):
        ...
        if self.state['Finishing Case Queue'] > 0:
            ...
            var = self.data['Cars'][first_car_in_queue][4] == 1
            self.data['Last Server Status']['Filer'] = self.state['Filer Server Status']
            self.data_server_calculater('Filer')
            self.state['Filer Server Status'] += 1
    ...
    def trace_excel_maker(self):
        ...
        columns.insert(0, 'Current Event')
        columns.insert(1, 'Clock')
        columns.extend(
...
series_data_opening = []
series_data_finishing = []
series_data_out = []
series_data_asses = []
indices_data = []

for rep in range(10):
    ICS = InsuranceCenterSimulation(simulation_time=5000)
    data, state, tr = ICS.simulation(trace_creator=True)
    df = ICS.trace_excel_maker()
    series_data_opening.append(df['Opening Case Queue'].tolist())
    series_data_finishing.append(df['Finishing Case Queue'].tolist())
    series_data_out.append(df['Outside the Area Queue'].tolist())
    series_data_asses.append(df['Assessment Queue'].tolist())
    indices_data.append(df['Clock'].tolist())
    plt.plot(df['Clock'], df['Opening Case Queue'])
min_index = min(min(indices) for indices in indices_data)
max_index = max(max(indices) for indices in indices_data)
common_indices = np.linspace(min_index, max_index, num=1000)

interpolated_series_opening = []
interpolated_series_finishing = []
interpolated_series_out = []
interpolated_series_asses = []
for series, indices in zip(series_data_opening, indices_data):
    interpolated_series_opening.append(np.interp(common_indices, indices, series))

for series, indices in zip(series_data_finishing, indices_data):
    interpolated_series_finishing.append(np.interp(common_indices, indices, series))

for series, indices in zip(series_data_out, indices_data):
    interpolated_series_out.append(np.interp(common_indices, indices, series))

for series, indices in zip(series_data_out, indices_data):
    interpolated_series_asses.append(np.interp(common_indices, indices, series))

average_series_opening = np.mean(interpolated_series_opening, axis=0)
average_series_finishing = np.mean(interpolated_series_finishing, axis=0)
average_series_out = np.mean(interpolated_series_out, axis=0)
average_series_asses = np.mean(interpolated_series_asses, axis=0)

end_time = time.time()
print('CPU Time: {0:.4f}'.format(end_time - start_time))
